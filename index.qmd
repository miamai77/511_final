...
-   Clean religious adherence data to remove outlier values (e.g. suspiciously high)
-   Load state-, metro-, and socioeconomic-level polling data from PRRI and Pew Research Center, on factors relating to RWA (e.g. "Is there concrete morality?" "Is law and order a major priority for you?" "Is society becoming too weak?")
-   Create a regression model using caret, based on responses to these questions, with state-level demographic data as training data, to calculate factors influencing such responses.
-   Predict county RWA values from this/
